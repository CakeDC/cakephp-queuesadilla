...
class MongoEngineTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        parent::setUp();
        $mongoUrl = getenv('MONGO_TEST_URL') ?: getenv('MONGO_URL');
        if (empty($mongoUrl)) {
            $this->markTestSkipped("Set MONGO_TEST_URL or MONGO_URL to run MongoEngine tests.");
        }
        if (!class_exists(Client::class)) {
            $this->markTestSkipped('Install ext-mongodb and mongodb/mongodb to run MongoEngine tests.');
        }
        $this->Logger = new NullLogger();
        $this->Engine = new MongoEngine($this->Logger, [
            'url' => $this->normalizeMongoTestUrl($mongoUrl),
        ]);
        $this->Engine->connect();
    }

    /**
     * Ensures the Mongo URL includes a database name for integration tests.
     *
     * @param string $mongoUrl Mongo connection URL.
     * @return string
     */
    protected function normalizeMongoTestUrl(string $mongoUrl): string
    {
        $path = parse_url($mongoUrl, PHP_URL_PATH);

        if ($path === null || $path === '' || $path === '/') {
            return rtrim($mongoUrl, '/') . '/queuesadilla_test';
        }
    ...
    public function testPopWithDelay()
    {
        ...
        $this->Engine->getCollection()->updateOne(
            ...
            [
                '$set' => ['delay_until' => (new \DateTime('+1 hour'))->getTimestamp()]
            ]
        );
        $actual = $this->Engine->pop([
            'queue' => 'test_push'
    ...
    public function testPopWithExpiresAt()
    {
        ...
        $this->Engine->getCollection()->updateOne(
            ...
            [
                '$set' => ['expires_at' => (new \DateTime('-1 hour'))->getTimestamp()]
            ]
        );
