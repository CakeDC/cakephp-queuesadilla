...
class SequentialWorkerTest extends TestCase
{
    ...
    public function testWork()
    {
        ...
        $Engine = $this->createNullEnginePopStub([
            [
                'class' => 'josegonzalez\Queuesadilla\invalid_work_test_callable',
            ],
            $this->Item,
            $this->ItemFail,
            $this->ItemException,
        ...
        $runtimeEngine = $this->createNullEnginePopStub([false]);
        $Worker = new SequentialWorker($runtimeEngine, null, ['maxRuntime' => 0]);
        $this->assertTrue($Worker->work());
    ...
    {
        $Engine = $this->createNullEnginePopStub([false]);

        $Worker = new SequentialWorker($Engine, null, ['maxIterations' => 1]);
        $this->assertTrue($Worker->work());
        $this->assertEquals(1, $Worker->stats()['empty']);
    }
    ...
    {
        $Engine = $this->createNullEnginePopStub([$this->ItemException]);

        $Worker = new SequentialWorker($Engine, null, ['maxIterations' => 1]);
        $this->assertTrue($Worker->work());
        $this->assertEquals(1, $Worker->stats()['exception']);
    }
    ...
    {
        $Engine = $this->createNullEnginePopStub([$this->Item]);

        $Worker = new SequentialWorker($Engine, null, ['maxIterations' => 1]);
        $this->assertTrue($Worker->work());
        $this->assertEquals(1, $Worker->stats()['success']);
    }
}
